...
class InstanceImageGoal_Env:
    ...
    def step(self, action):
        ...
        # Get location change
        if self.args.environment == 'ai2thor':
            dx, dy, do = self._env._get_location_change()
    ...
    def get_agent_pose(self):
        if self.args.environment == 'habitat':
            ...
            # 直接调用 AI2Thor_Env 实例的方法
            # AI2Thor_Env 内部会处理从 AI2-THOR metadata 到标准格式的转换
            return self._env.get_agent_pose()
    ...
    def get_agent_location(self):
        ...
        if self.args.environment == 'ai2thor':
            # AI2Thor_Env 内部会处理从 AI2-THOR metadata 到标准格式的转换
            return self._env._get_agent_location()
