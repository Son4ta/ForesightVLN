...
def construct_envs(args):
    if args.environment == 'habitat':
        ...
        print("Constructing AI2-THOR environment and loading ProcTHOR dataset...")
        import prior
        # 从 args (配置文件) 中获取数据集名称和分割
        procthor_dataset = prior.load_dataset(args.dataset['name'])
        split = args.dataset['split']
        # 将加载的数据集分割赋值给 dataset 变量
        dataset = procthor_dataset[split]
        print(f"Loaded {len(dataset)} episodes from ProcTHOR '{split}' split.")
        env = InstanceImageGoal_Env(args=args, dataset=dataset)
        return env
